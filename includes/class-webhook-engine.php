		...
		$response  = wp_safe_remote_post(
			...
			array(
				...
				'headers'     => array(
					...
					'X-Flatsome-MCP-Event'         => $row['event'],
					'X-Flatsome-MCP-Delivery'      => (string) $delivery_id,
					'X-Flatsome-MCP-Timestamp'     => $timestamp,
					'X-Flatsome-MCP-Signature-256' => 'sha256=' . $signature,
				),
				'body'        => $payload,
			),
		);
