...
final class Admin {
	...
	public function render(): void {
		...
		?>
		<div class="wrap fmp-admin" data-fmp-admin>
			<div id="fmp-copy-status" class="screen-reader-text" aria-live="polite"></div>

			<header class="fmp-masthead">
				<div class="fmp-brand">
					<div class="fmp-brand__mark" aria-hidden="true">
						<span class="dashicons dashicons-admin-links"></span>
					</div>
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'WordPress agent operations', 'mindio-magic-mcp' ); ?></p>
						<h1><?php esc_html_e( 'Mindio Magic MCP', 'mindio-magic-mcp' ); ?></h1>
						<p class="fmp-masthead__summary"><?php esc_html_e( 'Securely connect AI agents, govern access, automate events, and monitor every tool call from one control plane.', 'mindio-magic-mcp' ); ?></p>
					</div>
				</div>
			...
			<nav class="fmp-tabs" aria-label="<?php esc_attr_e( 'Mindio Magic MCP sections', 'mindio-magic-mcp' ); ?>">
				<?php foreach ( $this->tabs() as $slug => $tab ) : ?>
					<a
						class="fmp-tab <?php echo $active_tab === $slug ? 'is-active' : ''; ?>"
			...
			<main id="fmp-console-panel" class="fmp-console" tabindex="-1">
				<?php
				if ( is_array( $flash ) ) {
			...
			<footer class="fmp-console-footer">
				<span>
					<?php
	...
	private function render_flash( array $flash ): void {
		...
		<div class="fmp-alert fmp-alert--<?php echo esc_attr( $type ); ?>" role="alert">
			...
			<div class="fmp-alert__content">
				...
				<?php if ( '' !== $secret ) : ?>
					<div class="fmp-secret">
						<label for="fmp-one-time-secret"><?php esc_html_e( 'One-time secret', 'mindio-magic-mcp' ); ?></label>
						<div class="fmp-copy-field">
							<input id="fmp-one-time-secret" class="fmp-control fmp-control--code" type="text" readonly value="<?php echo esc_attr( $secret ); ?>" dir="ltr">
							<?php $this->render_copy_button( 'fmp-one-time-secret', __( 'one-time secret', 'mindio-magic-mcp' ) ); ?>
						</div>
	...
	private function render_overview( array $settings ): void {
		...
		?>
		<section class="fmp-section-heading">
			<div>
				<p class="fmp-eyebrow"><?php esc_html_e( 'Control plane', 'mindio-magic-mcp' ); ?></p>
			...
			<a class="fmp-button fmp-button--primary" href="<?php echo esc_url( $this->tab_url( 'credentials' ) ); ?>">
				<span class="dashicons dashicons-plus-alt2" aria-hidden="true"></span>
				<?php esc_html_e( 'Create credential', 'mindio-magic-mcp' ); ?>
			</a>
		</section>

		<div class="fmp-metrics">
			<?php
		...
		<div class="fmp-layout fmp-layout--wide">
			<section class="fmp-card fmp-card--connection">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'Client configuration', 'mindio-magic-mcp' ); ?></p>
			...
			<aside class="fmp-card">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'Environment', 'mindio-magic-mcp' ); ?></p>
						<h3><?php esc_html_e( 'System readiness', 'mindio-magic-mcp' ); ?></h3>
					</div>
				</div>
				<div class="fmp-readiness">
					<?php
		...
		<div class="fmp-layout fmp-layout--equal">
			<section class="fmp-card">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'Deployment path', 'mindio-magic-mcp' ); ?></p>
			...
			<section class="fmp-card">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'Observability', 'mindio-magic-mcp' ); ?></p>
						<h3><?php esc_html_e( 'Latest tool activity', 'mindio-magic-mcp' ); ?></h3>
					</div>
					<a class="fmp-inline-link" href="<?php echo esc_url( $this->tab_url( 'activity' ) ); ?>"><?php esc_html_e( 'View all', 'mindio-magic-mcp' ); ?></a>
				</div>
				<?php if ( $activity ) : ?>
					<div class="fmp-activity-feed">
						<?php foreach ( array_slice( $activity, 0, 5 ) as $row ) : ?>
							<div class="fmp-activity-feed__item">
								<span class="fmp-result-dot fmp-result-dot--<?php echo ! empty( $row['success'] ) ? 'success' : 'error'; ?>" aria-hidden="true"></span>
								<div>
	...
	private function render_credentials(): void {
		...
		<section class="fmp-section-heading">
			<div>
				<p class="fmp-eyebrow"><?php esc_html_e( 'Identity and access', 'mindio-magic-mcp' ); ?></p>
		...
		<div class="fmp-layout fmp-layout--credentials">
			<section class="fmp-card">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'New credential', 'mindio-magic-mcp' ); ?></p>
				...
				<form class="fmp-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-loading-label="creating">
					...
					<div class="fmp-form-grid">
						<div class="fmp-field">
							<label for="fmp-key-user"><?php esc_html_e( 'WordPress user', 'mindio-magic-mcp' ); ?></label>
							<?php
						...
						<div class="fmp-field">
							<label for="fmp-key-scope"><?php esc_html_e( 'Permission scope', 'mindio-magic-mcp' ); ?></label>
							<select id="fmp-key-scope" class="fmp-control" name="scope">
					...
					<button class="fmp-button fmp-button--primary" type="submit" data-submit-label>
			...
			<aside class="fmp-card fmp-card--tinted">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'Least privilege', 'mindio-magic-mcp' ); ?></p>
						<h3><?php esc_html_e( 'Scope policy', 'mindio-magic-mcp' ); ?></h3>
					</div>
				</div>
				<div class="fmp-scope-guide">
				...
				<div class="fmp-callout fmp-callout--info">
		...
		<section class="fmp-card fmp-card--table">
			<div class="fmp-card__header">
				<div>
					<p class="fmp-eyebrow"><?php esc_html_e( 'API access', 'mindio-magic-mcp' ); ?></p>
							...
							<?php foreach ( $tokens as $token ) : ?>
								<tr>
									<td>
										...
										<code class="fmp-subvalue"><?php echo esc_html( 'fmp_' . (string) $token['id'] . '_••••' ); ?></code>
									</td>
		...
		<section class="fmp-card fmp-card--table">
			<div class="fmp-card__header">
				<div>
					<p class="fmp-eyebrow"><?php esc_html_e( 'OAuth 2.1', 'mindio-magic-mcp' ); ?></p>
	...
	private function render_webhooks(): void {
		...
		<section class="fmp-section-heading">
			<div>
				<p class="fmp-eyebrow"><?php esc_html_e( 'Event automation', 'mindio-magic-mcp' ); ?></p>
		...
		<div class="fmp-layout fmp-layout--webhooks">
			<section class="fmp-card">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'New subscription', 'mindio-magic-mcp' ); ?></p>
				...
				<form class="fmp-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-webhook-form data-loading-label="creating">
					...
					<div class="fmp-form-grid">
						<div class="fmp-field">
							...
							<input id="fmp-webhook-name" class="fmp-control" type="text" name="name" maxlength="200" required placeholder="<?php esc_attr_e( 'Automation platform', 'mindio-magic-mcp' ); ?>">
						</div>
						<div class="fmp-field">
							<label for="fmp-webhook-url"><?php esc_html_e( 'HTTPS endpoint URL', 'mindio-magic-mcp' ); ?></label>
							<input id="fmp-webhook-url" class="fmp-control" type="url" name="url" required inputmode="url" dir="ltr" placeholder="https://hooks.example.com/wordpress">
						</div>
					</div>
					<fieldset class="fmp-events">
						...
						<div class="fmp-event-grid">
							<?php foreach ( Webhook_Engine::EVENTS as $event ) : ?>
								<label class="fmp-event-option" for="<?php echo esc_attr( 'fmp-event-' . $event ); ?>">
									<input id="<?php echo esc_attr( 'fmp-event-' . $event ); ?>" type="checkbox" name="events[]" value="<?php echo esc_attr( $event ); ?>">
									<span class="fmp-event-option__check" aria-hidden="true"><span class="dashicons dashicons-yes"></span></span>
									<span>
						...
						<p class="fmp-field-error" data-webhook-error role="alert" hidden><?php esc_html_e( 'Select at least one event before creating the webhook.', 'mindio-magic-mcp' ); ?></p>
					</fieldset>
					<button class="fmp-button fmp-button--primary" type="submit" data-submit-label>
						<span class="dashicons dashicons-plus-alt2" aria-hidden="true"></span>
			...
			<aside class="fmp-card fmp-card--dark">
				<div class="fmp-card__header">
					<div>
						<p class="fmp-eyebrow"><?php esc_html_e( 'Delivery security', 'mindio-magic-mcp' ); ?></p>
		...
		<section class="fmp-card fmp-card--table">
			<div class="fmp-card__header">
				<div>
					<p class="fmp-eyebrow"><?php esc_html_e( 'Subscriptions', 'mindio-magic-mcp' ); ?></p>
							...
							<?php foreach ( $webhooks as $webhook ) : ?>
								<tr>
									<td>
										...
										<code class="fmp-subvalue"><?php echo esc_html( (string) $webhook['id'] ); ?></code>
									</td>
									<td><code class="fmp-url-value" title="<?php echo esc_attr( (string) $webhook['url'] ); ?>"><?php echo esc_html( (string) $webhook['url'] ); ?></code></td>
									<td><div class="fmp-pill-list"><?php foreach ( (array) $webhook['events'] as $event ) : ?><code><?php echo esc_html( (string) $event ); ?></code><?php endforeach; ?></div></td>
	...
	private function render_activity(): void {
		...
		<section class="fmp-section-heading">
			<div>
				<p class="fmp-eyebrow"><?php esc_html_e( 'Governance and diagnostics', 'mindio-magic-mcp' ); ?></p>
		...
		<section class="fmp-card fmp-card--table">
			<div class="fmp-card__header fmp-card__header--toolbar">
				<div>
					<p class="fmp-eyebrow"><?php esc_html_e( 'Audit trail', 'mindio-magic-mcp' ); ?></p>
				...
				<?php if ( $activity ) : ?>
					<div class="fmp-table-toolbar" data-table-controls="fmp-activity-table">
						<label class="fmp-search">
			...
			<?php if ( $activity ) : ?>
				<div class="fmp-table-wrap">
					<table id="fmp-activity-table" class="fmp-table">
						<thead><tr>
							...
							<?php foreach ( $activity as $row ) : ?>
								...
								<tr data-table-row data-status="<?php echo esc_attr( $status ); ?>" data-search="<?php echo esc_attr( $search ); ?>">
									...
									<td>
										...
										<span class="fmp-subvalue"><?php echo esc_html( '#' . (string) $row['user_id'] ); ?></span>
									</td>
									...
									<td><strong class="fmp-duration"><?php echo esc_html( number_format_i18n( (int) $row['duration_ms'] ) ); ?> ms</strong></td>
								</tr>
							<?php endforeach; ?>
							<tr class="fmp-filter-empty" data-table-empty hidden><td colspan="6"><?php esc_html_e( 'No records match your filters.', 'mindio-magic-mcp' ); ?></td></tr>
						</tbody>
					</table>
				</div>
			<?php else : ?>
		...
		<section class="fmp-card fmp-card--table">
			<div class="fmp-card__header">
				<div>
					<p class="fmp-eyebrow"><?php esc_html_e( 'Outbound automation', 'mindio-magic-mcp' ); ?></p>
							...
							<?php foreach ( $deliveries as $delivery ) : ?>
								...
								<tr>
									...
									<td>
										...
										<code class="fmp-subvalue"><?php echo esc_html( (string) $delivery['webhook_id'] ); ?></code>
									</td>
	...
	private function render_tools(): void {
		...
		?>
		<section class="fmp-section-heading fmp-section-heading--settings">
			<div>
				<p class="fmp-eyebrow"><?php esc_html_e( 'Capability governance', 'mindio-magic-mcp' ); ?></p>
			...
			<button class="fmp-button fmp-button--primary" type="submit" form="fmp-tools-form">
		...
		<form id="fmp-tools-form" class="fmp-tool-manager" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-tool-manager data-loading-label="saving">
			...
			<section class="fmp-card fmp-tool-controls" aria-label="<?php esc_attr_e( 'Tool filters and bulk actions', 'mindio-magic-mcp' ); ?>">
				<div class="fmp-tool-controls__filters">
					<label class="fmp-search fmp-search--tools" for="fmp-tool-search">
						<span class="dashicons dashicons-search" aria-hidden="true"></span>
						<input id="fmp-tool-search" type="search" data-tool-search placeholder="<?php esc_attr_e( 'Search tool name or description…', 'mindio-magic-mcp' ); ?>">
					</label>
					<label class="fmp-filter-field" for="fmp-tool-group-filter">
						<span class="screen-reader-text"><?php esc_html_e( 'Filter by tool group', 'mindio-magic-mcp' ); ?></span>
						<select id="fmp-tool-group-filter" class="fmp-control" data-tool-group-filter>
					...
					<output class="fmp-tool-count" data-tool-count aria-live="polite">
						<?php
						/* translators: 1: visible tool count, 2: total tool count. */
				...
				<div class="fmp-tool-controls__actions">
					<button class="fmp-button fmp-button--compact" type="button" data-tool-enable-all><?php esc_html_e( 'Enable all tools', 'mindio-magic-mcp' ); ?></button>
					<button class="fmp-button fmp-button--compact" type="button" data-tool-disable-all><?php esc_html_e( 'Disable all tools', 'mindio-magic-mcp' ); ?></button>
				</div>
			</section>

			<div class="fmp-callout fmp-callout--info fmp-tool-policy-note">
				<span class="dashicons dashicons-shield" aria-hidden="true"></span>
			...
			<div class="fmp-tool-groups">
				<?php foreach ( $groups as $key => $group ) : ?>
					<?php
					$group_id         = 'fmp-tool-group-' . sanitize_html_class( $key );
					...
					?>
					<section class="fmp-card fmp-tool-group" data-tool-group data-group-key="<?php echo esc_attr( $key ); ?>" data-search="<?php echo esc_attr( $group_search ); ?>" aria-labelledby="<?php echo esc_attr( $group_id ); ?>">
						<header class="fmp-tool-group__header">
							<div class="fmp-tool-group__identity">
								<span class="fmp-settings-icon"><span class="dashicons <?php echo esc_attr( sanitize_html_class( $group['icon'] ) ); ?>" aria-hidden="true"></span></span>
								<div>
									<h3 id="<?php echo esc_attr( $group_id ); ?>"><?php echo esc_html( $group['label'] ); ?></h3>
									<p><?php echo esc_html( $group['description'] ); ?></p>
								</div>
							</div>
							<div class="fmp-tool-group__policy">
								<span>
									<?php
								...
								<label class="fmp-group-toggle">
									<input type="checkbox" data-tool-group-toggle <?php checked( $group_all_on ); ?>>
									<span class="fmp-group-toggle__track" aria-hidden="true"><span></span></span>
									<strong><?php esc_html_e( 'Enable group', 'mindio-magic-mcp' ); ?></strong>
								</label>
							</div>
						</header>
						<div class="fmp-tool-list">
							<?php foreach ( $group['items'] as $tool ) : ?>
								<?php
								$tool_id             = 'fmp-tool-' . sanitize_html_class( $tool['name'] );
								...
								?>
								<article class="fmp-tool-row is-<?php echo esc_attr( $state ); ?> <?php echo $operation_total > 0 ? 'has-operations' : ''; ?>" data-tool-row data-search="<?php echo esc_attr( $search ); ?>" data-tool-state="<?php echo esc_attr( $state ); ?>">
									<div class="fmp-tool-row__main">
									<label class="fmp-tool-toggle" for="<?php echo esc_attr( $tool_id ); ?>" aria-label="<?php
									...
									?>">
											...
											<span class="fmp-tool-toggle__track" aria-hidden="true"><span></span></span>
										</label>
										<div class="fmp-tool-row__content">
											<div class="fmp-tool-row__title">
												<code dir="ltr"><?php echo esc_html( $tool['name'] ); ?></code>
												<?php if ( $operation_total > 0 ) : ?>
											<span class="fmp-tool-annotation">
												<?php
												/* translators: %s: number of integration operations. */
												...
												<?php elseif ( ! empty( $tool['destructive'] ) ) : ?>
													<span class="fmp-tool-annotation fmp-tool-annotation--danger"><?php esc_html_e( 'Destructive', 'mindio-magic-mcp' ); ?></span>
												<?php elseif ( ! empty( $tool['read_only'] ) ) : ?>
													<span class="fmp-tool-annotation"><?php esc_html_e( 'Read operation', 'mindio-magic-mcp' ); ?></span>
												<?php endif; ?>
											</div>
											<small id="<?php echo esc_attr( $description_id ); ?>"><?php echo esc_html( $tool['description'] ); ?></small>
										</div>
										<div class="fmp-tool-row__meta">
											<?php $this->render_scope_badge( $tool['scope'] ); ?>
											<span class="fmp-tool-state fmp-tool-state--<?php echo esc_attr( $state ); ?>" data-tool-status><?php echo esc_html( $state_label ); ?></span>
											<?php if ( $operation_total > 0 ) : ?>
												<button class="fmp-operation-disclosure" type="button" data-operation-disclosure aria-expanded="false" aria-controls="<?php echo esc_attr( $operation_panel_id ); ?>">
									...
									<?php if ( $operation_total > 0 ) : ?>
										<div id="<?php echo esc_attr( $operation_panel_id ); ?>" class="fmp-operation-panel" data-operation-panel hidden>
											<header class="fmp-operation-panel__header">
												<div>
												...
												<div class="fmp-operation-panel__actions">
											...
											<div class="fmp-operation-list">
												<?php foreach ( $tool_operations as $operation ) : ?>
													...
													?>
													<label class="fmp-operation-row is-<?php echo esc_attr( $operation_state ); ?>" for="<?php echo esc_attr( $operation_id ); ?>" data-operation-row data-operation-mode="<?php echo esc_attr( $operation['mode'] ); ?>">
														...
														<span class="fmp-operation-checkbox" aria-hidden="true"><span class="dashicons dashicons-yes"></span></span>
														<span class="fmp-operation-row__content">
														...
														<span class="fmp-operation-row__meta">
															<span class="fmp-operation-mode fmp-operation-mode--<?php echo esc_attr( $operation['mode'] ); ?>"><?php echo 'read' === $operation['mode'] ? esc_html__( 'Read', 'mindio-magic-mcp' ) : esc_html__( 'Write', 'mindio-magic-mcp' ); ?></span>
															<?php if ( ! empty( $operation['destructive'] ) ) : ?><span class="fmp-tool-annotation fmp-tool-annotation--danger"><?php esc_html_e( 'Destructive', 'mindio-magic-mcp' ); ?></span><?php endif; ?>
															<?php $this->render_scope_badge( $operation['scope'] ); ?>
															<span class="fmp-operation-state fmp-operation-state--<?php echo esc_attr( $operation_state ); ?>" data-operation-status><?php echo esc_html( $operation_state_label ); ?></span>
														</span>
													</label>
												<?php endforeach; ?>
											</div>
			...
			<div class="fmp-card fmp-tool-empty" data-tool-empty hidden>
			...
			<div class="fmp-save-bar">
				...
				<button class="fmp-button fmp-button--primary" type="submit">
					<span class="dashicons dashicons-saved" aria-hidden="true"></span>
					<span data-submit-label><?php esc_html_e( 'Save tool policy', 'mindio-magic-mcp' ); ?></span>
				</button>
			</div>
	...
	private function render_settings( array $settings ): void {
		...
		<section class="fmp-section-heading fmp-section-heading--settings">
			<div>
				<p class="fmp-eyebrow"><?php esc_html_e( 'Policy and runtime', 'mindio-magic-mcp' ); ?></p>
			...
			<button class="fmp-button fmp-button--primary" type="submit" form="fmp-settings-form" data-settings-submit>
				<span class="dashicons dashicons-saved" aria-hidden="true"></span>
		...
		<form id="fmp-settings-form" class="fmp-settings-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-loading-label="saving">
			...
			<section class="fmp-card fmp-settings-group">
				<div class="fmp-settings-group__intro">
					<span class="fmp-settings-icon"><span class="dashicons dashicons-admin-generic" aria-hidden="true"></span></span>
					<div>
				...
				<div class="fmp-settings-group__body">
					<?php $this->render_switch( 'fmp-enabled', 'enabled', ! empty( $settings['enabled'] ), __( 'Enable MCP requests', 'mindio-magic-mcp' ), __( 'Authenticated clients can discover and call tools through the MCP endpoint.', 'mindio-magic-mcp' ) ); ?>
					<div class="fmp-settings-grid">
						<div class="fmp-field">
							<label for="fmp-rate"><?php esc_html_e( 'Rate limit', 'mindio-magic-mcp' ); ?></label>
							<div class="fmp-unit-field"><input id="fmp-rate" class="fmp-control" type="number" min="5" max="1000" name="rate_limit" value="<?php echo esc_attr( (string) $settings['rate_limit'] ); ?>"><span><?php esc_html_e( 'req / minute', 'mindio-magic-mcp' ); ?></span></div>
							<small><?php esc_html_e( 'Applied independently to each credential.', 'mindio-magic-mcp' ); ?></small>
						</div>
						<div class="fmp-field">
							<label for="fmp-upload"><?php esc_html_e( 'Media upload limit', 'mindio-magic-mcp' ); ?></label>
							<div class="fmp-unit-field"><input id="fmp-upload" class="fmp-control" type="number" min="1" max="100" name="max_upload_mb" value="<?php echo esc_attr( (string) $settings['max_upload_mb'] ); ?>"><span>MB</span></div>
			...
			<section class="fmp-card fmp-settings-group">
				<div class="fmp-settings-group__intro">
					<span class="fmp-settings-icon"><span class="dashicons dashicons-shield-alt" aria-hidden="true"></span></span>
					<div>
				...
				<div class="fmp-settings-group__body">
					<div class="fmp-field">
						<label for="fmp-origins"><?php esc_html_e( 'Allowed browser origins', 'mindio-magic-mcp' ); ?></label>
						<textarea id="fmp-origins" class="fmp-control fmp-control--code" rows="5" name="allowed_origins" dir="ltr" placeholder="https://agent.example.com"><?php echo esc_textarea( implode( "\n", (array) $settings['allowed_origins'] ) ); ?></textarea>
			...
			<section class="fmp-card fmp-settings-group">
				<div class="fmp-settings-group__intro">
					<span class="fmp-settings-icon"><span class="dashicons dashicons-database" aria-hidden="true"></span></span>
					<div>
				...
				<div class="fmp-settings-group__body">
					<div class="fmp-settings-grid">
						<div class="fmp-field">
							<label for="fmp-audit-retention"><?php esc_html_e( 'Audit retention', 'mindio-magic-mcp' ); ?></label>
							<div class="fmp-unit-field"><input id="fmp-audit-retention" class="fmp-control" type="number" min="1" max="365" name="audit_retention_days" value="<?php echo esc_attr( (string) $settings['audit_retention_days'] ); ?>"><span><?php esc_html_e( 'days', 'mindio-magic-mcp' ); ?></span></div>
						</div>
						<div class="fmp-field">
							<label for="fmp-webhook-retention"><?php esc_html_e( 'Webhook retention', 'mindio-magic-mcp' ); ?></label>
							<div class="fmp-unit-field"><input id="fmp-webhook-retention" class="fmp-control" type="number" min="1" max="365" name="webhook_retention_days" value="<?php echo esc_attr( (string) $settings['webhook_retention_days'] ); ?>"><span><?php esc_html_e( 'days', 'mindio-magic-mcp' ); ?></span></div>
						</div>
					</div>
				</div>
			</section>
			...
			<section class="fmp-card fmp-settings-group">
				<div class="fmp-settings-group__intro">
					<span class="fmp-settings-icon"><span class="dashicons dashicons-editor-code" aria-hidden="true"></span></span>
					<div>
				...
				<div class="fmp-settings-group__body">
					...
					<div class="fmp-callout fmp-callout--warning">
			...
			<section class="fmp-card fmp-settings-group fmp-settings-group--danger">
				<div class="fmp-settings-group__intro">
					<span class="fmp-settings-icon"><span class="dashicons dashicons-trash" aria-hidden="true"></span></span>
					<div>
				...
				<div class="fmp-settings-group__body">
					<?php $this->render_switch( 'fmp-delete-data', 'delete_on_uninstall', ! empty( $settings['delete_on_uninstall'] ), __( 'Delete all plugin data on uninstall', 'mindio-magic-mcp' ), __( 'Permanently remove credentials, OAuth registrations, webhooks, configuration, and logs when the plugin is deleted.', 'mindio-magic-mcp' ), true ); ?>
				</div>
			</section>
	...
	private function render_metric( string $label, string $value, string $detail, string $icon, string $tone ): void {
		?>
		<article class="fmp-metric fmp-metric--<?php echo esc_attr( sanitize_html_class( $tone ) ); ?>">
			<span class="fmp-metric__icon"><span class="dashicons <?php echo esc_attr( sanitize_html_class( $icon ) ); ?>" aria-hidden="true"></span></span>
			<div>
	...
	private function render_endpoint( string $id, string $label, string $value, string $description ): void {
		?>
		<div class="fmp-endpoint">
			<div class="fmp-endpoint__meta">
				<strong><?php echo esc_html( $label ); ?></strong>
				<small><?php echo esc_html( $description ); ?></small>
			</div>
			<div class="fmp-inline-copy fmp-inline-copy--endpoint">
	...
	private function render_copy_button( string $target_id, string $accessible_label, bool $compact = false ): void {
		?>
		<button
			class="fmp-copy-button <?php echo $compact ? 'fmp-copy-button--compact' : ''; ?>"
			type="button"
		...
		>
			...
			<span class="fmp-copy-button__label"><?php esc_html_e( 'Copy', 'mindio-magic-mcp' ); ?></span>
		</button>
		<?php
	}
	...
	private function render_readiness_row( string $label, string $value, string $tone ): void {
		?>
		<div class="fmp-readiness__row">
			<span><?php echo esc_html( $label ); ?></span>
			<div>
				<strong><?php echo esc_html( $value ); ?></strong>
				<span class="fmp-readiness__indicator fmp-readiness__indicator--<?php echo esc_attr( sanitize_html_class( $tone ) ); ?>" aria-hidden="true"></span>
			</div>
		</div>
		<?php
	}
	...
	private function render_status_badge( string $label, string $tone ): void {
		...
		?>
		<span class="fmp-badge fmp-badge--<?php echo esc_attr( $tone ); ?>">
			<span aria-hidden="true"></span>
	...
	private function render_empty_state( string $icon, string $title, string $description ): void {
		?>
		<div class="fmp-empty">
			<span class="fmp-empty__icon"><span class="dashicons <?php echo esc_attr( sanitize_html_class( $icon ) ); ?>" aria-hidden="true"></span></span>
	...
	private function render_switch( string $id, string $name, bool $checked, string $title, string $description, bool $danger = false ): void {
		?>
		<label class="fmp-switch <?php echo $danger ? 'fmp-switch--danger' : ''; ?>" for="<?php echo esc_attr( $id ); ?>">
			<input id="<?php echo esc_attr( $id ); ?>" type="checkbox" name="<?php echo esc_attr( $name ); ?>" value="1" <?php checked( $checked ); ?>>
			<span class="fmp-switch__track" aria-hidden="true"><span></span></span>
			<span class="fmp-switch__content">
				<strong><?php echo esc_html( $title ); ?></strong>
				<small><?php echo esc_html( $description ); ?></small>
			</span>
		</label>
