...
function mindio_component_assert( bool $condition, string $message ): void {
	if ( ! $condition ) {
...
mindio_component_assert( ! empty( $admins ), 'The WordPress fixture needs an administrator.' );
...
mindio_component_assert( empty( $upload['error'] ), 'Could not create the component-test image.' );
...
mindio_component_assert( $media_id > 0, 'Could not register the component-test image.' );
...
try {
	...
	mindio_component_assert( '3.20.7' === $discovery['flatsome_version'], 'Unexpected Flatsome test version.' );
	mindio_component_assert( 29 === count( $discovery['components'] ), 'The component catalog is incomplete.' );
	...
	foreach ( $fixtures as $type => $fixture ) {
		mindio_component_assert( true === $validator->validate( $fixture, $schema ), 'The strict schema rejected ' . $type . '.' );
		$result = $renderer->render_element_fragment( $fixture, 'rtl' );
		mindio_component_assert( ! is_wp_error( $result ), 'Rendering failed for ' . $type . ': ' . ( is_wp_error( $result ) ? $result->get_error_message() : '' ) );
		mindio_component_assert( str_contains( $result['content'], '[' . $expected_shortcodes[ $type ] ), 'The native shortcode is missing for ' . $type . '.' );
		mindio_component_assert( $result['render_report']['native_count'] >= 1 && 0 === $result['render_report']['fallback_count'], 'The native report is invalid for ' . $type . '.' );
		$rendered_html = do_shortcode( $result['content'] );
		mindio_component_assert( is_string( $rendered_html ), 'Flatsome did not render ' . $type . '.' );
	}

	$text_result = $renderer->render_element_fragment( $fixtures['text'] );
	mindio_component_assert( ! is_wp_error( $text_result ) && str_contains( $text_result['content'], '<strong>body</strong>' ), 'Valid UX text did not render.' );
	...
	mindio_component_assert( is_wp_error( $heading_text ) && 'text_markup_forbidden' === $heading_text->get_error_code(), 'UX text accepted heading markup.' );
	mindio_component_assert( is_wp_error( $shortcode_text ) && 'text_markup_forbidden' === $shortcode_text->get_error_code(), 'UX text accepted shortcode markup.' );
	mindio_component_assert( is_wp_error( $validator->validate( array( 'type' => 'text', 'html' => '<p>Legacy</p>' ), $schema ) ), 'The strict schema accepted legacy text.html.' );
	...
	mindio_component_assert( ! is_wp_error( $html_result ) && 1 === $html_result['render_report']['fallback_count'], 'Explicit HTML fallback was not reported.' );
	mindio_component_assert( ! str_contains( $html_result['content'], '<script' ) && ! str_contains( $html_result['content'], 'onclick=' ) && str_contains( $html_result['content'], '&#91;title&#93;' ), 'Explicit HTML fallback was not sanitized.' );
	...
	mindio_component_assert( ! is_wp_error( $missing_result ) && 1 === $missing_result['render_report']['fallback_count'] && str_contains( $missing_result['content'], '[ux_html' ) && str_contains( $missing_result['content'], '<h2>' ), 'Unavailable shortcode fallback failed.' );

	if ( ! class_exists( 'WooCommerce' ) ) {
		$products = $renderer->render_element_fragment( array( 'type' => 'products', 'limit' => 4 ) );
		mindio_component_assert( is_wp_error( $products ) && 'component_dependency_missing' === $products->get_error_code(), 'Missing WooCommerce dependency did not fail clearly.' );
	}
